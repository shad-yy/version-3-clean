  ...
  return (
    <div className="relative z-10 space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-gradient-to-br from-[#1a1a28] to-[#12121d] border border-[#2a2a3e] rounded-2xl p-6 flex flex-col justify-between shadow-lg">
          <div>
          ...
          <Link
            href="/scores"
            className="inline-flex items-center justify-center gap-2 bg-[#00e676] text-black font-extrabold text-sm py-4 px-6 rounded-xl hover:bg-[#00ff87] transition-all hover:scale-[1.02] shadow-[0_0_20px_rgba(0,230,118,0.2)]"
          >
      ...
      <div className="border-t border-[#1a1a2a] pt-6">
        <h4 className="font-bold text-white text-sm mb-4">Why watch on Smart Live TV?</h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={item.t} className={`${CARD_BG_50} border border-[#2a2a3e] p-4 rounded-xl`}>
              <h5 className="text-xs font-bold text-[#00e676] mb-1.5">{item.t}</h5>
              <p className="text-xs text-gray-300 leading-relaxed">{item.d}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="relative z-10">
        <div className={`text-center py-16 ${CARD_BG_30} border border-white/5 rounded-2xl`}>
          <BarChart3 className="w-12 h-12 mx-auto mb-4 text-[#00e676] opacity-40" />
          <p className="text-sm text-gray-500">Stats will be available after the match begins.</p>
        </div>
      </div>
    )
  }
          ...
          return (
            <div key={name} className="space-y-2">
              <div className="flex justify-between text-xs font-bold uppercase tracking-wide">
                ...
                <span className="text-white text-center font-black tracking-widest">{name}</span>
              ...
              <div className={`h-2.5 w-full ${CARD_BG} rounded-full overflow-hidden flex`}>
                <div
                  ...
                  style={{ width: `${homePct}%` }}
                />
                <div className="h-full bg-[#12121a]" style={{ width: `${100 - homePct - awayPct}%` }} />
                <div
...
function TimelineView({ timeline, homeTeam, awayTeam }: TimelineViewProps) {
  return (
    <div className="relative z-10 space-y-6">
      <h3 className="text-lg font-bold text-white mb-6">Match Events</h3>
      {timeline.length === 0 ? (
        <div className={`text-center py-10 text-gray-500 ${CARD_BG_30} border border-white/5 rounded-2xl`}>
          <Clock className="w-12 h-12 mx-auto mb-4 text-[#00e676] opacity-60" />
          <p className="text-sm">Timeline events will appear as goals, bookings, and substitutions occur.</p>
        </div>
      ) : (
        <div className="relative border-l-2 border-[#1a1a2a] ml-4 pl-6 space-y-8">
          {timeline.map((event, idx) => {
            const isHome = event.strHome === "yes" || event.strTeam === homeTeam
            return (
              <div key={event.idTimeline || idx} className="relative">
                <div className="absolute -left-[31px] top-1 w-4 h-4 rounded-full bg-[#12121a] border-2 border-emerald-500 flex items-center justify-center">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                </div>

                <div className={`${CARD_BG_50} border border-white/5 rounded-xl p-4 flex justify-between items-center hover:border-[#00e676]/30 transition-colors`}>
                  <div>
                    <span className="text-xs font-bold text-[#00e676] mr-2">{event.strTime || "—"}&apos;</span>
                    <span className="text-sm font-bold text-white">{event.strEvent}</span>
                    {event.strPlayer && (
                  ...
                  <span className="text-xs font-extrabold uppercase px-2.5 py-1 rounded bg-[#1a1a2a] text-gray-300 border border-white/5 max-w-[120px] truncate">
...
function TeamColumn({
  ...
  return (
    <div>
      ...
      {lineup.length === 0 ? (
        <div className={`p-6 text-center text-gray-500 ${CARD_BG_30} border border-white/5 rounded-2xl`}>
        ...
        <div className="space-y-2">
          {lineup.map((player) => (
            <div
              key={player.idPlayer}
              className={`flex items-center gap-3 ${CARD_BG_50} p-2.5 rounded-lg border border-white/5 hover:border-[#00e676]/20 transition-colors`}
            >
              ...
              <div>
                <p className="text-sm font-bold text-white">{player.strPlayer}</p>
                <p className="text-[10px] text-gray-400 uppercase font-semibold">{player.strPosition || "Player"}</p>
              </div>
            </div>
          ))}
        </div>
...
export function MatchTabs({
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex bg-[#12121a]/95 border border-[#1a1a2a] p-1.5 rounded-xl mb-8 overflow-x-auto scrollbar-none gap-1.5 max-w-xl mx-auto backdrop-blur-md shadow-[0_4px_20px_rgba(0,0,0,0.4)]">
        {tabs.map((tab) => {
          const Icon = tab.icon
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="bg-gradient-to-br from-[#12121a] via-[#12121a] to-[#0d0d14] border border-[#1b1b2f] rounded-2xl p-6 md:p-8 shadow-2xl relative overflow-hidden"
        >
